  ...
  return <>
    ...
    <main id="top">
      ...
      <section className="section news" id="notícias"><SectionHead eyebrow="ÚLTIMAS DA ZONA" title="Notícias sem contraindicações." />
        <div className="news-grid">{content.news.map((item, i) => <article className={`news-card ${i === 0 ? 'featured' : ''}`} key={item.id}>{item.image ? <div className="news-image-missing"><span>Proposta de logótipo do Francis</span><small>imagem original a adicionar</small></div> : <div className="news-art"><span>{item.category}</span></div>}<div className="news-copy"><div><span>{item.category}</span><time>{new Date(`${item.date}T12:00:00`).toLocaleDateString('pt-PT', {day:'2-digit', month:'short'})}</time></div><h3>{item.title}</h3><p>{item.summary}</p>{item.id === 'shirts' && <a href={links.instagram} target="_blank" rel="noreferrer">Dar informações <Camera size={14}/></a>}</div></article>)}</div>
      </section>
